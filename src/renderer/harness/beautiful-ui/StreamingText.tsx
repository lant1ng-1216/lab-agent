...
/* ─────────────────────────────────────────────────────────
 * STREAMING TEXT
 * Gallery: word-timer choreography + sources/follow-ups.
 * Live (`mode="live"`): bind to real agent text — caret +
 * chrome only when callers pass real data.
 * ───────────────────────────────────────────────────────── */
...
function LiveBody({
  ...
  const lastLenRef = useRef(text.length);
  const lastAtRef = useRef(Date.now());
  const caughtUpRef = useRef(false);

  // Soft typewriter: follow new text; when complete, drain remaining then notify
  useEffect(() => {
    ...
    if (text === shown) {
      if (complete && text.length > 0 && !caughtUpRef.current) {
    ...
    // Small chunks: reveal immediately (token deltas)
    if (delta.length <= 12 && !complete) {
      setShown(text);
      lastLenRef.current = text.length;
      lastAtRef.current = Date.now();
      setStalled(false);
    ...
    let i = 0;
    const step = complete
      ? Math.max(3, Math.ceil(delta.length / 28))
      : Math.max(2, Math.ceil(delta.length / 18));
    const id = window.setInterval(() => {
      i = Math.min(delta.length, i + step);
      const next = text.slice(0, shown.length + i);
      setShown(next);
      if (i >= delta.length) {
        window.clearInterval(id);
        if (complete && !caughtUpRef.current) {
          caughtUpRef.current = true;
          onCatchUp?.();
        }
      }
  ...
  // Stall indicator when no growth for ~420ms while incomplete
  useEffect(() => {
    if (complete) {
      setStalled(false);
      return;
    ...
    const tick = window.setInterval(() => {
      ...
      if (Date.now() - lastAtRef.current > 420) setStalled(true);
    }, 160);
    return () => window.clearInterval(tick);
  }, [text, complete, shown.length]);

  const draining = complete && shown.length < text.length;

  return (
    <div className="w-full min-w-0 overflow-visible">
      <div className="text-[14.5px] leading-[1.65] text-ink">
        {shown ? <MarkdownBody text={shown} streaming={!complete || draining} /> : null}
        {!complete || draining ? (
          <span className="mt-1 inline-flex min-h-4 items-center">
            {!stalled || draining ? (
              <span
                className="inline-block h-3 w-0.5 rounded-full bg-ink"
                style={{ animation: "fade-in 150ms ease-out both" }}
              />
            ) : (
              <span className="ml-1 inline-flex items-center">
                <PixelLoader variant="Drive" cell={3} />
              </span>
            )}
          </span>
        ) : null}
      </div>
      {complete && !draining && shown ? (
        <div
...
export default function StreamingText({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [count, done, loop, mode]);

  if (mode === "live") {
    return (
      <div className={fill ? "w-full" : "min-h-[4rem] w-full max-w-95"}>
        <LiveBody text={liveText} complete={complete} onCatchUp={onDone} />
      </div>
    );
  }
