...
function LoaderGrid({
  ...
  return (
    <span
      aria-hidden
      className="grid shrink-0 gap-[1.5px]"
      style={{ gridTemplateColumns: `repeat(3, ${cell}px)` }}
    >
      {delays.map((delay, index) => (
        <span
          key={index}
          className={`bg-ink ${round ? "rounded-full" : "rounded-[1px]"}`}
          style={{
            width: cell,
            height: cell,
...
/** Compact 3×3 pixel loader for inline stream / stall indicators */
export function PixelLoader({
  ...
  return (
    <span className="inline-flex items-center align-middle" role="status" aria-label={label}>
      <LoaderGrid delays={delays} dur={dur} round={round} cell={cell} />
    </span>
  );
}
