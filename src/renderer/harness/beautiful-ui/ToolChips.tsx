...
export type ToolStep = {
  ...
  /** Full path for preview open */
  filePath?: string;
};

export type ToolDiff = { file: string; add: number; del: number; /** display name */ label?: string };
...
export default function ToolChips({
  ...
  onOpenChange,
  onToggleRow,
  /** Skip STEP_MS march — show all real steps/diffs as they exist */
  revealAll = false,
  hideMore = false,
  onFileOpen,
}: {
  ...
  revealAll?: boolean;
  hideMore?: boolean;
  /** Click file chip / mono chip → open preview */
  ...
  const visibleStep = revealAll ? Math.max(steps.length, total) : step;
    ...
    <div className={`min-h-[220px] w-full max-w-80 pb-1${className ? ` ${className}` : ""}`}>
      ...
      <div className="grid transition-[grid-template-rows,opacity] duration-300" style={{ gridTemplateRows: open ? "1fr" : "0fr", opacity: open ? 1 : 0 }}>
        ...
        <div className="mt-1.5 flex flex-col gap-1">
          {steps.slice(0, visibleStep).map((row) => {
            ...
            <div key={row.label} style={{ animation: "fade-up 300ms cubic-bezier(0.23,1,0.32,1) both" }}>
              ...
              >
                ...
                <span
                  role={row.filePath && onFileOpen ? "link" : undefined}
                  onClick={
                    row.filePath && onFileOpen
                      ? (e) => {
                          e.stopPropagation();
                          onFileOpen(row.filePath!);
                        }
                      : undefined
                  }
      ...
      {/* file-diff chips — only when real diffs exist */}
      {diffs.length > 0 && visibleStep >= total && (
        <div className="mt-2.5 flex max-w-full flex-wrap gap-1.5 border-t border-line pt-2.5">
          {diffs.map((d, i) => (
            ...
            >
              ...
              >
                <span className="min-w-0 truncate">{d.label || d.file.replace(/^.*[/\\]/, "")}</span>
