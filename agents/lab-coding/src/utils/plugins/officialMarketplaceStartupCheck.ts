 ...
 * Lab Agent: Anthropic official marketplace is always disabled unless explicitly
 * re-enabled with LAB_USE_ANTHROPIC_MARKETPLACE=1 (escape hatch).
 */
export function isOfficialMarketplaceAutoInstallDisabled(): boolean {
  if (isEnvTruthy(process.env.LAB_USE_ANTHROPIC_MARKETPLACE)) {
    return isEnvTruthy(
      process.env.CLAUDE_CODE_DISABLE_OFFICIAL_MARKETPLACE_AUTOINSTALL,
    )
  }
  // Default for Lab Agent / Lab Code: do not auto-install Anthropic market
  return true
}
