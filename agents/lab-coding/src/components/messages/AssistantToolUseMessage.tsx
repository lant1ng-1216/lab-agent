...
import { LAB_MARK_MIN_WIDTH, LAB_MARK_TOOL_RUNNING } from '../../constants/figures.js';
...
export function AssistantToolUseMessage(t0) {
  ...
  const t6 = stringWidth(userFacingToolName) + (shouldShowDot ? LAB_MARK_MIN_WIDTH : 0);
  let t7;
  if ($[31] !== isQueued || $[32] !== isResolved || $[33] !== lookups.erroredToolUseIDs || $[34] !== param.id || $[35] !== shouldAnimate || $[36] !== shouldShowDot) {
    t7 = shouldShowDot && (isQueued ? <Box minWidth={LAB_MARK_MIN_WIDTH}><Text dimColor={isQueued}>{LAB_MARK_TOOL_RUNNING}</Text></Box> : <ToolUseLoader shouldAnimate={shouldAnimate} isUnresolved={!isResolved} isError={lookups.erroredToolUseIDs.has(param.id)} />);
    $[31] = isQueued;
    $[32] = isResolved;
